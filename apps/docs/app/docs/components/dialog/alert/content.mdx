...
앱 전체 기본 Alert 컴포넌트를 등록합니다. 자세한 내용은 [Dialog](/docs/components/dialog#modalroot)를 참고하세요.

```tsx
import { ModalRoot } from '@woon-ui/dialog'
import { MyAlert } from './ui/Alert'

<ModalRoot components={{ alert: MyAlert }} />
```
...
<Callout type="tip">
  **렌더 우선순위** (높음 → 낮음): `render` prop → `ModalRoot components.alert` → 내장 기본 UI
</Callout>
...
`@woon-ui/dialog/css/alert`는 alert 전용 data 속성과 base motion을 함께 포함합니다. 버튼 색상 등 특정 값만 바꾸려면 선택자로 override하세요.
