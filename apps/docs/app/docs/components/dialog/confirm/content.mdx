...
앱 루트에 `DialogRuntime`이 있으면 어디서든 호출할 수 있습니다.
...
앱 전체 기본 Confirm 컴포넌트를 등록합니다. 자세한 내용은 [Dialog](/docs/components/dialog#dialogruntime)를 참고하세요.
...
`render` 옵션 또는 `DialogRuntime components.confirm`에 등록한 컴포넌트가 받는 컨텍스트입니다.
