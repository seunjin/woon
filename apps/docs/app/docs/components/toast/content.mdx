...
WoonProvider에 `toastPlugin()`을 추가하면 렌더 위치와 설정을 연결할 수 있습니다.

## 설정

### toastPlugin

`WoonProvider`에 `toastPlugin()`을 추가하면 렌더 위치와 설정을 연결할 수 있습니다.
...
| `duration` | `number` | `5000` | 자동 닫힘 시간 (ms). 최솟값 2000ms |

`duration: Infinity`로 설정하면 사용자가 직접 닫을 때까지 유지됩니다.
