...
```ts
import { toast, toastPlugin, Toaster } from '@woon-ui/react/toast'
```
...
```tsx
import { Toaster } from '@woon-ui/react/toast'
...
```tsx
import { toast } from '@woon-ui/react/toast'
...
`@woon-ui/toast/css`로 기본 스타일이 적용됩니다. 특정 값만 바꾸려면 선택자로 override하세요.
