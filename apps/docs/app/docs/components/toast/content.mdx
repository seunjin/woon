...
export const toc = [
  ...
  { id: 'toaster', label: 'Toaster', depth: 3 },
...
`toast()`는 어디서든 호출할 수 있는 모듈 레벨 함수입니다.
앱 루트에 `Toaster`를 한 번 렌더하면 됩니다.
...
`Toaster`에 `position`, `maxVisible`, `zIndex`, `render`를 전달해 동작과 기본 UI를 설정할 수 있습니다.
